 ...
 * @version 4.0.0
NM: Modified
- Added quantity arrow buttons class
- Added Thumbnail to grouped-products table */

defined( 'ABSPATH' ) || exit;

global $product, $post, $nm_theme_options;

// NM: Quantity arrow buttons class
$nm_quantity_arrows_class = ( $nm_theme_options['qty_arrows_grouped'] ) ? ' qty-show' : '';

do_action( 'woocommerce_before_add_to_cart_form' ); ?>

<form class="cart grouped_form" action="<?php echo esc_url( apply_filters( 'woocommerce_add_to_cart_form_action', $product->get_permalink() ) ); ?>" method="post" enctype='multipart/form-data'>
	<table cellspacing="0" class="woocommerce-grouped-product-list group_table<?php echo esc_attr( $nm_quantity_arrows_class ); ?>">
		<tbody>
			<?php
			...
			foreach ( $grouped_products as $grouped_product_child ) {
				...
				foreach ( $grouped_product_columns as $column_id ) {
					...
					switch ( $column_id ) {
                        // NM: Product thumbnail
                        case 'thumbnail':
							$value = get_the_post_thumbnail( $grouped_product_child->get_id(), 'woocommerce_thumbnail' );
                            break;
                        // /NM
						case 'quantity':
							ob_start();
